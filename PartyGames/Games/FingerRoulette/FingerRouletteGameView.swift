...
struct FingerRouletteGameView: View {
    @StateObject private var vm = FingerRouletteViewModel()
    ...
    private var presetPicker: some View {
        HStack(spacing: 8) {
            ForEach(Array(vm.presets.enumerated()), id: \.offset) { idx, preset in
                ...
                .padding(.vertical, 8)
                .background(vm.selectedPreset == idx ? AppTheme.secondary : AppTheme.surface)
                .clipShape(Capsule())
            }
        }
        .padding(.vertical, 10)
    ...
    private var resultPopup: some View {
        if vm.showResult, let result = vm.resultSegment {
            ...
            .animation(.spring(response: 0.4, dampingFraction: 0.6), value: vm.showResult)
            .padding(.top, 16)
        }
    }
}
